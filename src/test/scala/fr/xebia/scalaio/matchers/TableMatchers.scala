package fr.xebia.scalaio.matchers
...
final class MatchTableWord[Actual] extends ClassicMatchers {
  ...
  def apply[A, B, C, D](properties: (CheckColumn[A], CheckColumn[B], CheckColumn[C], CheckColumn[D]), expectations: (A, B, C, D)*): Matcher[Traversable[Actual]] = new Matcher[Traversable[Actual]] {
